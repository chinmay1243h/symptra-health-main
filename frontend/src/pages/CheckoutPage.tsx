...
import { Loader2, CheckCircle, ShoppingCart, Truck, CreditCard } from 'lucide-react';
...
const CheckoutPage = () => {
  ...
  const handlePlaceOrder = async (e: React.FormEvent) => {
    ...
    if (!user) { // Should be caught by useEffect, but double-check
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        navigate(`/order-confirmation/${data.data._id}`, { replace: true }); // Redirect to confirmation page
      } else {
  ...
  if (cartItems.length === 0 && !orderSuccess) {
    // This state should be handled by useEffect redirecting to products
    return null; 
  }
              ...
              <CardContent>
                <RadioGroup value={paymentMethod} onValueChange={setPaymentMethod}>
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <Label htmlFor="stripe">Credit Card (Stripe Simulated)</Label>
                  </div>
                </RadioGroup>
              </CardContent>
            </Card>
