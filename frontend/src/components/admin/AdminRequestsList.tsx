...
import { 
  ...
  XIcon, 
  RefreshCw,
  EyeIcon // For viewing request details
} from 'lucide-react';
...
type RequestType = 'article_approval' | 'product_approval' | 'user_registration' | 'appointment_booking' | 'free_consultation';
...
type RequestItem = {
  ...
  reviewedBy?: { // Optional, as it might not be reviewed yet
    _id: string;
    name: string;
    email: string;
  };
...
const AdminRequestsList = () => {
  ...
  const [filterStatus, setFilterStatus] = useState<RequestStatus | 'all'>('pending'); // Default filter to pending
  const [filterType, setFilterType] = useState<RequestType | 'all'>('all'); // Default filter to all types
  ...
  const fetchRequests = async () => {
    ...
    try {
      ...
      if (filterStatus === 'pending') {
        url = `${API_BASE_URL}/requests/pending`;
      } 
      ...
      if (data.success) {
        ...
        // Apply client-side filtering for status if not using /pending endpoint
  ...
  // Re-fetch requests when filters change
  useEffect(() => {
    fetchRequests();
  ...
  const getRequestTypeBadge = (type: RequestType) => {
    ...
    return <span className={`px-2 py-1 rounded text-xs font-medium ${colorClass}`}>
      {type.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())} {/* Format type string */}
    </span>;
  };
                      ...
                      {request.status === 'pending' && (
                        <>
                          <Button 
                            ...
                            size="sm" 
                            onClick={() => {
                              setCurrentRequest(request); // Set current request for dialog
                              setReviewNotes(''); // Clear previous notes
                              setDialogOpen(true); // Open dialog for processing
                            }}
